...
GrammarParser::ParsingTable GrammarParser::ConstructPredictiveParsingTable()
    const {
  ParsingTable table;
  ...
  for (const auto& pair : this->grammar) {
    ...
    for (const auto& token : pair.second) {
      if (IsNonTerminal(token)) {
        ...
        if (this->firstMembers.at(token).find(RESERVE_WORD_LAMBDA) ==
            this->firstMembers.at(token).end()) {
          lastLambda = false;
          break;
        } else {
          lastLambda = true;
...
std::unordered_set<std::string> GrammarParser::GetFirstFromRightSide(
    const std::vector<std::string>& vector) const {
...
bool GrammarParser::ContainsPlaceholderFollow(
