...
void walkProgram(std::ostream& out, const Parser::Token& token) {
  ...
  if (token.type == "<dec>") {
    const auto id = token.children.at(0).getToken();     // <id>
    const auto prime = token.children.at(1).getToken();  // <dec-prime>

    out << extractLiterals(id);
  ...
  if (token.type == "<dec-prime>") {
    const auto id = token.children.at(1).getToken();     // <id>
  ...
  if (token.type == "<write-prime>") {
    if (token.children.size() == 3) {
      ...
      const auto identifier = token.children.at(2).getToken();  // <identifier>
